...
- ⚠️ **改了客户端代码看不到效果**：`Ctrl+B`（macOS 为 `Cmd+B`）只做本地编译，**真机调试的包必须重新点「真机调试」/「预览」重新扫码才会重推**；保险起见先「工具 → 清除缓存」，并把手机上的小游戏**杀进程**再扫码。
- 📄 日常该双击哪个脚本、改完代码要不要重建 —— 刘老师本机有一份速查
  （**不在仓库内**，属个人机器专属）：`D:\workbuddy\projects\测试速查-浏览器与真机.md`。
  仓库内的对应内容见本章 §5 / §7 / §8。
