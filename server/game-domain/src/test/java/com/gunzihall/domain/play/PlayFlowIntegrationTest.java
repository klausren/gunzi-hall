...
class PlayFlowIntegrationTest {
    ...
    /**
     * 收圈时分牌明细要一并留档（客户端「闲家得分」展开面板的数据源）。
     *
     * <p>只留分值 &gt; 0 的牌（5/10/K）：整圈 4 张都塞给客户端既费带宽也没有展示价值，
     * 玩家要确认的是"这些分具体是哪几张牌"。
     */
    @Test
    void trickPointCardsRecordedForWinnerOnly() {
        GameRoom room = playingRoom(
                List.of(Card.of(Suit.HEART, 7), Card.of(Suit.CLUB, 3)),   // 北：♥7 + ♣3
                List.of(Card.of(Suit.HEART, 13), Card.of(Suit.CLUB, 4)),  // 东：♥K + ♣4
                List.of(Card.of(Suit.HEART, 10), Card.of(Suit.CLUB, 5)),  // 南：♥10 + ♣5
        ...
        // ♥K + ♥10 归东家所在的 B 队；♣5 是南家垫的，不归赢家 → 不入账
        List<Card> taken = room.takenPointCards().getOrDefault(Team.B, List.of());
        // 明细按**出牌顺序**留档（北 ♥7 → 西 ♣6 → 南 ♥10 → 东 ♥K），只留分值 > 0 的牌
        assertEquals(List.of(Card.of(Suit.HEART, 10), Card.of(Suit.HEART, 13)), taken,
                "赢家队伍收到的分牌明细（按出牌顺序，仅 5/10/K）");
        assertTrue(room.takenPointCards().getOrDefault(Team.A, List.of()).isEmpty(),
                "输家队伍不应有分牌入账");
    }
    ...
    void outOfTurnRejected() {
        ...
        assertTrue(play(room, 1L, List.of(Card.of(Suit.HEART, 7))));
        assertFalse(play(room, 3L, List.of(Card.of(Suit.HEART, 10))), "没轮到南家（北的下家是西）");
        assertTrue(play(room, 4L, List.of(Card.of(Suit.CLUB, 6))), "西才是北的下家");
        assertTrue(play(room, 3L, List.of(Card.of(Suit.HEART, 10))));
        assertTrue(play(room, 2L, List.of(Card.of(Suit.HEART, 13))));
    }
    ...
    void liveRuleStillEnforcesSuitFollow() {
        ...
        assertTrue(play(room, 1L, List.of(Card.of(Suit.HEART, 8), Card.of(Suit.HEART, 8))));
        // 逆时针：北 → 西 → 南 → 东。西、南都没 ♥，垫副牌合法
        assertTrue(play(room, 4L, List.of(Card.of(Suit.CLUB, 6), Card.of(Suit.CLUB, 7))));
        assertTrue(play(room, 3L, List.of(Card.of(Suit.CLUB, 4), Card.of(Suit.CLUB, 5))));
        // 东家唯一的 ♥ 必须跟出，两张全垫副牌被拒（必须是"轮到东家"时的拒绝，不能是被轮次挡下）
